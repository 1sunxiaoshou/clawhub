...
const DEFAULT_DESCRIPTION = "Deep Skill Hub — a fast skill registry for agents, with vector search.";
...
export function buildSkillMeta(source: SkillMetaSource): SkillMeta {
  ...
  const title = `${displayName} — Deep Skill Hub`;
  const description =
    summary || (owner ? `Agent skill by @${owner} on Deep Skill Hub.` : DEFAULT_DESCRIPTION);
